...
**Note:** The auth module is different from the [authz module](/v0.50/build/modules/authz).
...
##### Nested multisig transactions[​](#nested-multisig-transactions "Direct link to Nested multisig transactions")
...
```
# First aggregate signatures of the multisig participantsimd tx multi-sign transaction.json ms1 ms1p1sig.json ms1p2sig.json --signature-only --skip-signature-verification > ms1sig.json# Then use the aggregated signatures and the other signatures to sign the final transactionsimd tx multi-sign transaction.json k1ms1 k1sig.json ms1sig.json --skip-signature-verification
```
