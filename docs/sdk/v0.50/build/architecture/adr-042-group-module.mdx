...
* Key rotation is not possible, although this can be solved with [account rekeying](adr-034-account-rekeying).
...
It's meant to be used with other access control modules such as [`x/feegrant`](/docs/sdk/v0.50/adr-029-fee-grant-module) ans [`x/authz`](adr-030-authz-module) to simplify key management for individuals and organizations.
...
those "sub-accounts" using the [`x/authz` module](adr-030-authz-module).
...
Inter-module communication introduced by [ADR-033](adr-033-protobuf-inter-module-comm) can be used to route a proposal's messages using the `DerivedModuleKey` corresponding to the proposal's group account.
