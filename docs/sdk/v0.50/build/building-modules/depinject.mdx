...
[`depinject`](/v0.50/build/packages/depinject) is used to wire any module in `app.go`. All core modules are already configured to support dependency injection.
...
The App Wiring is done in `app_config.go` / `app.yaml` and `app_v2.go` and is explained in detail in the [overview of `app_v2.go`](/v0.50/build/building-apps/app-go-v2).
