...
The `InvariantRegistry` is therefore typically instantiated by instantiating the `keeper` of the `crisis` module in the [application's constructor function](/v0.50/learn/beginner/app-anatomy#constructor-function).

`Invariant`s can be checked manually via [`message`s](/v0.50/build/building-modules/messages-and-queries), but most often they are checked automatically at the end of each block. Here is an example from the `crisis` module:
