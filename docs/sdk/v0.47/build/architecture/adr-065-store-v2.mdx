...
There was a previous attempt to refactor the storage layer described in [ADR-040](/v0.47/build/architecture/adr-040-storage-and-smt-state-commitments). However, this approach mainly stems on the short comings of IAVL and various performance issues around it. While there was a (partial) implementation of [ADR-040](/v0.47/build/architecture/adr-040-storage-and-smt-state-commitments), it was never adopted for a variety of reasons, such as the reliance on using an SMT, which was more in a research phase, and some design choices that couldn't be fully agreed upon, such as the snap-shotting mechanism that would result in massive state bloat.

## Decision[​](#decision "Direct link to Decision")

We propose to build upon some of the great ideas introduced in [ADR-040](/v0.47/build/architecture/adr-040-storage-and-smt-state-commitments), while being a bit more flexible with the underlying implementations and overall less intrusive. Specifically, we propose to:
...
Furthermore, we will keep the IAVL is the backing [commitment](https://cryptography.fandom.com/wiki/Commitment_scheme) store for the time being. While we might not fully settle on the use of IAVL in the long term, we do not have strong empirical evidence to suggest a better alternative. Given that the SDK provides interfaces for stores, it should be sufficient to change the backing commitment store in the future should evidence arise to warrant a better alternative. However there is promising work being done to IAVL that should result in significant performance improvement \[1,2].

### Separating SS and SC[​](#separating-ss-and-sc "Direct link to Separating SS and SC")
...
#### State Commitment (SC)[​](#state-commitment-sc "Direct link to State Commitment (SC)")
...
#### State Storage (SS)[​](#state-storage-ss "Direct link to State Storage (SS)")
...
Note, this is not problematic for the state machine itself because when a query is made, the RMS will automatically direct the query correctly (see [Queries](#queries)).
...
### Atomic Commitment[​](#atomic-commitment "Direct link to Atomic Commitment")
...
### Module Storage Control[​](#module-storage-control "Direct link to Module Storage Control")
...
### Historical State Proofs[​](#historical-state-proofs "Direct link to Historical State Proofs")
...
### Physical DB Backends[​](#physical-db-backends "Direct link to Physical DB Backends")
...
* \[1] [https://github.com/cosmos/iavl/pull/676](https://github.com/cosmos/iavl/pull/676)
* \[2] [https://github.com/cosmos/iavl/pull/664](https://github.com/cosmos/iavl/pull/664)
* \[3] [https://github.com/cosmos/cosmos-sdk/issues/14990](https://github.com/cosmos/cosmos-sdk/issues/14990)
