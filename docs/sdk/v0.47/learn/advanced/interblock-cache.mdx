...
## Overview and basic concepts[​](#overview-and-basic-concepts "Direct link to Overview and basic concepts")
...
## System model and properties[​](#system-model-and-properties "Direct link to System model and properties")
...
### General design[​](#general-design "Direct link to General design")
...
```
type CommitKVStoreCacheManager interface{    cacheSize uint    caches map[string]CommitKVStore}
```
...
```
type CommitKVStoreCache interface{    store CommitKVStore    cache Cache}
```
...
#### CommitKVCacheManager[​](#commitkvcachemanager "Direct link to CommitKVCacheManager")
...
```
func NewCommitKVStoreCacheManager(size uint) CommitKVStoreCacheManager {    manager = CommitKVStoreCacheManager{size, make(map[string]CommitKVStore)}    return manager}
```
...
```
func GetStoreCache(    manager CommitKVStoreCacheManager,    storeKey string,    store CommitKVStore) CommitKVStore {    if manager.caches.has(storeKey) {        return manager.caches.get(storeKey)    } else {        cache = CommitKVStoreCacheManager{store, manager.cacheSize}        manager.set(storeKey, cache)        return cache    }}
```
...
```
func Unwrap(    manager CommitKVStoreCacheManager,    storeKey string) CommitKVStore {    if manager.caches.has(storeKey) {        cache = manager.caches.get(storeKey)        return cache.store    } else {        return nil    }}
```
...
```
function Reset(manager CommitKVStoreCacheManager) {    for (let storeKey of manager.caches.keys()) {        manager.caches.delete(storeKey)    }}
```

#### CommitKVStoreCache[​](#commitkvstorecache "Direct link to CommitKVStoreCache")
...
```
func NewCommitKVStoreCache(    store CommitKVStore,    size uint) CommitKVStoreCache {    KVCache = CommitKVStoreCache{store, NewCache(size)}    return KVCache}
```
...
```
func Get(    KVCache CommitKVStoreCache,    key string) []byte {    valueCache, success := KVCache.cache.Get(key)    if success {        // cache hit        return valueCache    } else {        // cache miss        valueStore = KVCache.store.Get(key)        KVCache.cache.Add(key, valueStore)        return valueStore    }}
```
...
```
func Set(    KVCache CommitKVStoreCache,    key string,    value []byte) {    KVCache.cache.Add(key, value)    KVCache.store.Set(key, value)}
```
...
```
func Delete(    KVCache CommitKVStoreCache,    key string) {    KVCache.cache.Remove(key)    KVCache.store.Delete(key)}
```
...
```
func CacheWrap(    KVCache CommitKVStoreCache) {         return CacheKV.NewStore(KVCache)}
```
