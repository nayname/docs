...
In addition to the commands for [running a node](/v0.53/user/run-node/run-node), the `simd` binary also includes a `testnet` command that allows you to start a simulated test network in-process or to initialize files for a simulated test network that runs in a separate process.

## Initialize Files[​](#initialize-files "Direct link to Initialize Files")
...
### gentxs[​](#gentxs "Direct link to gentxs")
...
## Start Testnet[​](#start-testnet "Direct link to Start Testnet")
...
## Testnet Options[​](#testnet-options "Direct link to Testnet Options")
