...
<Note>
  * [gRPC, REST and CometBFT Endpoints](/v0.53/learn/advanced/grpc_rest)
  * [Running a Node](/v0.53/user/run-node/run-node)
</Note>
...
The list of all available REST endpoints is available as a Swagger specification file, it can be viewed at `localhost:1317/swagger`. Make sure that the `api.swagger` field is set to true in your [`app.toml`](/v0.53/user/run-node/run-node#configuring-the-node-using-apptoml-and-configtoml) file.
