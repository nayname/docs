...
<Note>
  A Protobuf `Msg` service processes [messages](/v0.53/build/building-modules/messages-and-queries#messages). Protobuf `Msg` services are specific to the module in which they are defined, and only process messages defined within the said module. They are called from `BaseApp` during [`DeliverTx`](/v0.53/learn/advanced/baseapp#delivertx).
</Note>
...
As further described in [ADR 031](/v0.53/build/architecture/adr-031-msg-service), this approach has the advantage of clearly specifying return types and generating server and client code.
...
When possible, the existing module's [`Keeper`](/v0.53/build/building-modules/keeper) should implement `MsgServer`, otherwise a `msgServer` struct that embeds the `Keeper` can be created, typically in `./keeper/msg_server.go`:
...
After the validation is successful, the `msgServer` method uses the [`keeper`](/v0.53/build/building-modules/keeper) functions to access the state and perform a state transition.

### Events[​](#events "Direct link to Events")

Before returning, `msgServer` methods generally emit one or more [events](/v0.53/learn/advanced/events) by using the `EventManager` held in the `ctx`. Use the new `EmitTypedEvent` function that uses protobuf-based event types:
...
These events are relayed back to the underlying consensus engine and can be used by service providers to implement services around the application. Click [here](/v0.53/learn/advanced/events) to learn more about events.
...
![Transaction flow](/images/v0.53/build/building-modules/cosmos/cosmos-sdk/release/v0.46.x/docs/uml/svg/transaction_flow.svg)

## Telemetry[​](#telemetry "Direct link to Telemetry")

New [telemetry metrics](/v0.53/learn/advanced/telemetry) can be created from `msgServer` methods when handling messages.
