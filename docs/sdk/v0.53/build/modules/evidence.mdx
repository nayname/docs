...
```
// Handler defines an agnostic Evidence handler. The handler is responsible// for executing all corresponding business logic necessary for verifying the// evidence as valid. In addition, the Handler may execute any necessary// slashing and potential jailing.type Handler func(context.Context, Evidence) error
```
...
v0.50.x/proto/cosmos/evidence/v1beta1/evidence.proto

```
loading...
```

[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/tree/release/v0.50.x/proto/cosmos/evidence/v1beta1/evidence.proto#L12-L32)
...
v0.50.x/x/evidence/keeper/infraction.go

```
loading...
```

[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/tree/release/v0.50.x/x/evidence/keeper/infraction.go#L26-L140)

**Note:** The slashing, jailing, and tombstoning calls are delegated through the `x/slashing` module that emits informative events and finally delegates calls to the `x/staking` module. See documentation on slashing and jailing in [State Transitions](/v0.53/build/modules/staking#state-transitions).
