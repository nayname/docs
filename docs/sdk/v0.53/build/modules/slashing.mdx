...
### Tombstone Caps[​](#tombstone-caps "Direct link to Tombstone Caps")
...
### Infraction Timelines[​](#infraction-timelines "Direct link to Infraction Timelines")
...
*\[* : timeline start *]* : timeline end *Cn* : infraction `n` committed *Dn* : infraction `n` discovered *Vb* : validator bonded *Vu* : validator unbonded

#### Single Double Sign Infraction[​](#single-double-sign-infraction "Direct link to Single Double Sign Infraction")
...
#### Multiple Double Sign Infractions[​](#multiple-double-sign-infractions "Direct link to Multiple Double Sign Infractions")
...
### Signing Info (Liveness)[​](#signing-info-liveness "Direct link to Signing Info (Liveness)")
...
Proposers are incentivized to include precommits from all validators in the CometBFT `LastCommitInfo` by receiving additional fees proportional to the difference between the voting power included in the `LastCommitInfo` and +2/3 (see [fee distribution](/v0.53/build/modules/distribution#begin-block)).

```
type LastCommitInfo struct {    Round int32    Votes []VoteInfo}
```
...
### Unjail[​](#unjail "Direct link to Unjail")
...
### Liveness Tracking[​](#liveness-tracking "Direct link to Liveness Tracking")
...
### Staking hooks[​](#staking-hooks "Direct link to Staking hooks")
...
### Validator Bonded[​](#validator-bonded "Direct link to Validator Bonded")
...
```
onValidatorBonded(address sdk.ValAddress)  signingInfo, found = GetValidatorSigningInfo(address)  if !found {    signingInfo = ValidatorSigningInfo {      StartHeight         : CurrentHeight,      IndexOffset         : 0,      JailedUntil         : time.Unix(0, 0),      Tombstone           : false,      MissedBloskCounter  : 0    } else {      signingInfo.StartHeight = CurrentHeight    }    setValidatorSigningInfo(signingInfo)  }  return
```
...
### MsgServer[​](#msgserver "Direct link to MsgServer")

#### MsgUnjail[​](#msgunjail "Direct link to MsgUnjail")
...
### BeginBlocker: HandleValidatorSignature[​](#beginblocker-handlevalidatorsignature "Direct link to BeginBlocker: HandleValidatorSignature")
...
#### Slash[​](#slash "Direct link to Slash")

* same as `"slash"` event from `HandleValidatorSignature`, but without the `jailed` attribute.

#### Jail[​](#jail "Direct link to Jail")

| Type  | Attribute Key | Attribute Value     |
| ----- | ------------- | ------------------- |
| slash | jailed        | \{validatorAddress} |

## Staking Tombstone[​](#staking-tombstone "Direct link to Staking Tombstone")

### Abstract[​](#abstract-1 "Direct link to Abstract")
...
#### Proposal: infinite jail[​](#proposal-infinite-jail "Direct link to Proposal: infinite jail")
...
#### Single slashing amount[​](#single-slashing-amount "Direct link to Single slashing amount")
...
```
downtime_jail_duration: 600smin_signed_per_window: "0.500000000000000000"signed_blocks_window: "100"slash_fraction_double_sign: "0.050000000000000000"slash_fraction_downtime: "0.010000000000000000"
```

#### signing-info[​](#signing-info "Direct link to signing-info")
...
```
address: cosmosvalcons1nrqsld3aw6lh6t082frdqc84uwxn0t958cindex_offset: "2068"jailed_until: "1970-01-01T00:00:00Z"missed_blocks_counter: "0"start_height: "0"tombstoned: false
```

#### signing-infos[​](#signing-infos "Direct link to signing-infos")
...
```
info:- address: cosmosvalcons1nrqsld3aw6lh6t082frdqc84uwxn0t958c  index_offset: "2075"  jailed_until: "1970-01-01T00:00:00Z"  missed_blocks_counter: "0"  start_height: "0"  tombstoned: falsepagination:  next_key: null  total: "0"
```
...
#### unjail[​](#unjail-1 "Direct link to unjail")
...
```
{  "params": {    "signedBlocksWindow": "100",    "minSignedPerWindow": "NTAwMDAwMDAwMDAwMDAwMDAw",    "downtimeJailDuration": "600s",    "slashFractionDoubleSign": "NTAwMDAwMDAwMDAwMDAwMDA=",    "slashFractionDowntime": "MTAwMDAwMDAwMDAwMDAwMDA="  }}
```

#### SigningInfo[​](#signinginfo "Direct link to SigningInfo")
...
```
{  "valSigningInfo": {    "address": "cosmosvalcons1nrqsld3aw6lh6t082frdqc84uwxn0t958c",    "indexOffset": "3493",    "jailedUntil": "1970-01-01T00:00:00Z"  }}
```

#### SigningInfos[​](#signinginfos "Direct link to SigningInfos")
...
```
{  "info": [    {      "address": "cosmosvalcons1nrqslkwd3pz096lh6t082frdqc84uwxn0t958c",      "indexOffset": "2467",      "jailedUntil": "1970-01-01T00:00:00Z"    }  ],  "pagination": {    "total": "1"  }}
```
...
```
{  "params": {    "signed_blocks_window": "100",    "min_signed_per_window": "0.500000000000000000",    "downtime_jail_duration": "600s",    "slash_fraction_double_sign": "0.050000000000000000",    "slash_fraction_downtime": "0.010000000000000000"}
```

#### signing\_info[​](#signing_info "Direct link to signing_info")
...
```
{  "val_signing_info": {    "address": "cosmosvalcons1nrqslkwd3pz096lh6t082frdqc84uwxn0t958c",    "start_height": "0",    "index_offset": "4184",    "jailed_until": "1970-01-01T00:00:00Z",    "tombstoned": false,    "missed_blocks_counter": "0"  }}
```

#### signing\_infos[​](#signing_infos "Direct link to signing_infos")
...
```
{  "info": [    {      "address": "cosmosvalcons1nrqslkwd3pz096lh6t082frdqc84uwxn0t958c",      "start_height": "0",      "index_offset": "4169",      "jailed_until": "1970-01-01T00:00:00Z",      "tombstoned": false,      "missed_blocks_counter": "0"    }  ],  "pagination": {    "next_key": null,    "total": "1"  }}
```
