...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L17-L20)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L285-L317)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L34-L38)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L22-L32)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L115-L147)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L155-L225)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/rootmulti/store.go#L56-L82)

The `rootMulti.Store` is a base-layer multistore built around a `db` on top of which multiple `KVStores` can be mounted, and is the default multistore store used in [`baseapp`](/v0.53/learn/advanced/baseapp).
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/cachemulti/store.go#L20-L34)
...
Individual `KVStore`s are used by modules to manage a subset of the global state. `KVStores` can be accessed by objects that hold a specific key. This `key` should only be exposed to the [`keeper`](/v0.53/build/building-modules/keeper) of the module that defines the store.

`CommitKVStore`s are declared by proxy of their respective `key` and mounted on the application's [multistore](#multistore) in the [main application file](/v0.53/learn/beginner/app-anatomy#core-application-file). In the same file, the `key` is also passed to the module's `keeper` that is responsible for managing the store.
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/store.go#L227-L264)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/x/bank/keeper/view.go#L121-L137)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/iavl/store.go#L36-L41)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/dbadapter/store.go#L13-L16)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/transient/store.go#L16-L19)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/x/params/types/subspace.go#L22-L32)

Transient stores are typically accessed via the [`context`](/v0.53/learn/advanced/context) via the `TransientStore()` method:
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/types/context.go#L347-L350)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/cachekv/store.go#L26-L36)
...
Cosmos SDK applications use [`gas`](/v0.53/learn/beginner/gas-fees) to track resources usage and prevent spam. [`GasKv.Store`](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/gaskv/store.go) is a `KVStore` wrapper that enables automatic gas consumption each time a read or write to the store is made. It is the solution of choice to track storage usage in Cosmos SDK applications.
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/gaskv/store.go#L11-L17)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/types/gas.go#L219-L228)

By default, all `KVStores` are wrapped in `GasKv.Stores` when retrieved. This is done in the `KVStore()` method of the [`context`](/v0.53/learn/advanced/context):
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/types/context.go#L342-L345)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/tracekv/store.go#L20-L43)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/prefix/store.go#L15-L21)
...
[See full example on GitHub](https://github.com/cosmos/cosmos-sdk/blob/v0.53.0/store/listenkv/store.go#L11-L18)
