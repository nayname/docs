...
The WERC20 precompile provides a standard ERC20 interface to native Cosmos tokens through Cosmos EVM's **[Single Token Representation](/docs/evm/single-token-representation)** architecture. Unlike traditional wrapped tokens that maintain separate balances, WERC20 gives smart contracts direct access to native bank module balances through familiar ERC20 methods.

**Key Concept**: *"Schrodinger's Wrap"* ATOM and WATOM are not separate tokens—they are two different interfaces to the same token stored in the bank module.
