...
The mempool exposes Ethereum-compatible RPC methods for querying transaction pool state. See the [JSON-RPC Methods documentation](/docs/v0.4.x/api-reference/ethereum-json-rpc/methods#txpool-methods) for detailed API reference:
...
You can also explore these methods interactively using the [RPC Explorer](/docs/v0.4.x/api-reference/ethereum-json-rpc/rpc-explorer).

## Integration

<Note>
For chain developers looking to integrate the mempool into their Cosmos SDK chain, see the [EVM Mempool Integration Guide](/docs/v0.4.x/documentation/integration/mempool-integration) for complete setup instructions.
</Note>
