...
<Tabs>
  ...
  <Tab title="Chain Configuration">
    Dynamic chain configuration system for runtime parameter management.

    ## Configuration Methods

    ### CLI Flags (Highest Priority)

    Override any configuration parameter at runtime:
    ...
    Create `config/chain.toml` for persistent settings:
    ...
    export EVMD_EXTENDED_DENOM="custom"
    export EVMD_DISPLAY_DENOM="CUSTOM"
    export EVMD_DECIMALS="18"
    export EVMD_COIN_TYPE="60"
    ```
    ...
    Parameters are resolved in this order (highest to lowest):

    1. **CLI Flags** - Runtime command-line arguments
    2. **Configuration Files** - `config/chain.toml` settings
    ...
    evmd start --config /etc/evmd/config
    ```

    ### Testing Setup
    ```bash
    # Testnet with isolated parameters
    evmd testnet \
      --v 4 \
      --output-dir ./testnet \
      --chain-id test-evm-999 \
      --evm-chain-id 999 \
      --denom utest \
      --coin-type 1
    ```
    ...
    evmd config validate

    # Test specific parameter resolution
    evmd config get chain.id
    evmd config get chain.evm-chain-id
    ```

    ## Legacy Migration

    **Pre-v0.5.0 (Static Configuration):**
    ```go
    // Hard-coded in source
    const ChainID = "local-1"
    const Denom = "atest"
    ```

    **v0.5.0+ (Dynamic Configuration):**
    ```bash
    # Runtime configuration
    evmd start --chain-id local-1 --denom atest
    ```

    <Warning>
    **Breaking Change**: v0.5.0 removes static configuration constants. Use the dynamic configuration system for all chain parameters.
    </Warning>

    <Note>
      **Configuration Location**: Chain configuration files should be placed in the node's config directory alongside `app.toml` and `config.toml`.
    </Note>
  </Tab>
</Tabs>