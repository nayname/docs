...
<Tabs>
  ...
  <Tab title="EVM">
    ...
    <ParamField body="min-tip" type="string" default="0">
      Minimum priority fee (tip) required for transaction inclusion.

      Set to 0 to allow zero-fee transactions for development.

      **Flag:** `--evm.min-tip`
      **Source:** `config/server_app_options.go` via `GetMinTip()`
      **Implementation:** `server/flags/flags.go:EVMMinTip`
    </ParamField>
  </Tab>
