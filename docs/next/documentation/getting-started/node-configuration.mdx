...
<Tabs>
  ...
  <Tab title="EVM">
    ...
    <ParamField body="min-tip" type="number" default="0">
      Minimum priority fee (tip) required for transaction inclusion.

      Set to 0 to disable minimum tip requirement.

      **Flag:** `srvflags.EVMMinTip` 
  ...
  <Tab title="CLI Flags">
    ...
    <ParamField body="--evm.min-tip" type="number">
      Minimum priority fee (tip) for transaction inclusion
    </ParamField>
  ...
  <Tab title="app.toml">
    ...
    # MinTip is the minimum priority fee for transaction inclusion (uint64 format).
    min-tip = 0
