...
<footnote>
\*\*This will no longer be the case with the upcoming release of the [app-side mempool](https://www.notion.so/skip-protocol/EXT-Cosmos-EVM-Appside-Mempool-21e8c17818cb80028f28cfa440d7a7e7)
</footnote>
...
| `admin_*` | Node administration | Administrative functions handled differently in Cosmos SDK |
| `miner_*` | Mining operations | No mining in Tendermint BFT consensus |
...
Cosmos EVM focuses on implementing the core `eth_*`, `web3_*`, `net_*`, `txpool_*`, `personal_*`, and `debug_*` namespaces that are essential for Ethereum compatibility and developer tooling.
