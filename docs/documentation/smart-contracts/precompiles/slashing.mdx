...
**Note**: The slashing `getParams()` function may return empty data if slashing parameters are not configured on the devnet. The cURL method will still demonstrate the correct function call format.
