...
- **Local Queue Size**: Monitor local queue size to prevent memory issues
- **Promotion Frequency**: Balance promotion interval with network latency
- **Fee Estimation**: Use dynamic fee estimation for optimal inclusion
- **Batch Limits**: Consider practical limits when batching transactions

## Migration Guide

For chains upgrading to use the app-side mempool:

1. **Review Configuration** - Ensure your chain configuration supports the app-side mempool features
2. **Test in Devnet** - Thoroughly test transaction handling patterns in a development environment
3. **Update Client Code** - Update any custom client code to handle the new transaction patterns
4. **Monitor Performance** - Monitor mempool performance and adjust configuration as needed
...
|-------|-------|----------|
| Transactions stuck in local queue | Missing dependency transactions | Submit missing nonce transactions |
| High memory usage | Large local queue | Reduce `max-local-txs` setting |
| Transactions not gossiped | Nonce gaps | Expected behavior; transactions promoted when ready |
| RBF not working | Insufficient fee increase | Ensure significant fee increase (typically 10%+) |

### Debugging Commands

```bash
# Check mempool status
evmd query mempool status

# View local queue transactions
evmd query mempool local-queue

# Check specific transaction
evmd query mempool tx <hash>

# Monitor promotion events
evmd debug mempool promotions
```
...
The app-side mempool is implemented in:
- **Core Logic**: `x/evm/mempool/mempool.go`
- **CheckTx Handler**: `x/evm/ante/mempool_handler.go`
- **Promotion Logic**: `x/evm/mempool/promoter.go`
- **Configuration**: `x/evm/types/config.go`

### Related Specifications

- [GitHub Issue #417](https://github.com/cosmos/evm/issues/417) - Full technical specification
- [Pull Request #387](https://github.com/cosmos/evm/pull/387) - Implementation details

## Future Enhancements

Planned improvements for future releases:

- **Production Readiness**: Hardening for mainnet deployment
- **Advanced Priority**: Multi-dimensional fee priority mechanisms
- **Cross-Chain Support**: IBC-aware transaction ordering
- **MEV Protection**: Built-in MEV mitigation strategies
- **Analytics Dashboard**: Real-time mempool visualization tools

<Note>
This feature is under active development. Join the discussion on [GitHub](https://github.com/cosmos/evm) or [Discord](https://discord.gg/interchain) to provide feedback and stay updated on progress.
</Note>