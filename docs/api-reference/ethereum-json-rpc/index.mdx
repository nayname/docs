...
<Warning>
  ...
  * `eth_createAccessList` - EIP-2930 access lists
  * `eth_fillTransaction` - Transaction filling utility
  * `eth_resend` / `eth_pendingTransactions` - Transaction pool management
  * `personal_sendTransaction` / `personal_ecRecover` - Some personal namespace methods
  * All `miner_*` methods - Mining not applicable with Tendermint consensus
