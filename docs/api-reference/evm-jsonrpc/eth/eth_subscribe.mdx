...
Subscribe to particular events via WebSocket

### Parameters

1. `subscription_type` - Parameter description
2. `params` - Parameter description
...
// Example call
const result = await web3.eth.subscribe(newHeads, <params>);
console.log(result);
```
...
// Example call
const result = await provider.send('eth_subscribe', ["newHeads", "<params>"]);
console.log(result);
```
...
// Example call
result = w3.manager.request_blocking('eth_subscribe', ['newHeads', '<params>'])
print(result)
```
</CodeGroup>
