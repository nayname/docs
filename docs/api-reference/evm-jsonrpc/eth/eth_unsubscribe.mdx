...
Unsubscribe from particular subscription

### Parameters

1. `subscription_id` - Parameter description
...
// Example call
const result = await web3.eth.unsubscribe(0x1);
console.log(result);
```
...
// Example call
const result = await provider.send('eth_unsubscribe', ["0x1"]);
console.log(result);
```
...
// Example call
result = w3.manager.request_blocking('eth_unsubscribe', ['0x1'])
print(result)
```
</CodeGroup>
