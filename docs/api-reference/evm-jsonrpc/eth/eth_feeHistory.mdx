---
title: "eth_feeHistory"
description: "Returns fee history"
---

## Overview

Returns fee history

### Parameters

1. `block_count` - Parameter description
2. `newest_block` - Parameter description
3. `reward_percentiles` - Parameter description

### Returns

Fee history object

## Examples

### Request

<CodeGroup>
```bash cURL
curl -X POST https://evm-rpc.cosmos.network \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
    "method": "eth_feeHistory",
    "params": ["0x4", "latest", [25, 50, 75]],
    "id": 1
  }'
```

```javascript JavaScript (Web3.js)
const Web3 = require('web3');
const web3 = new Web3('https://evm-rpc.cosmos.network');

// Example call
const result = await web3.eth.feeHistory(0x4, latest, [25, 50, 75]);
console.log(result);
```

```javascript JavaScript (Ethers.js)
const { ethers } = require('ethers');
const provider = new ethers.providers.JsonRpcProvider('https://evm-rpc.cosmos.network');

// Example call
const result = await provider.send('eth_feeHistory', ["0x4", "latest", [25, 50, 75]]);
console.log(result);
```

```python Python (Web3.py)
from web3 import Web3

w3 = Web3(Web3.HTTPProvider('https://evm-rpc.cosmos.network'))

// Example call
result = w3.manager.request_blocking('eth_feeHistory', ['0x4', 'latest', [25, 50, 75]])
print(result)
```
</CodeGroup>

### Response

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": "// Method-specific result"
}
```

### Error Response

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "error": {
    "code": -32600,
    "message": "Invalid Request"
  }
}
```

## Notes

- This method is compatible with the Ethereum JSON-RPC specification
- Available on both mainnet and testnet endpoints
- Rate limits may apply on public endpoints

## Related Methods

- [`eth_accounts`](./eth_accounts)
- [`eth_getBalance`](./eth_getBalance)
- [`eth_getCode`](./eth_getCode)

## See Also

- [Complete JSON-RPC API Reference](../index)
- [EVM Development Guide](/docs/evm/developers)
- [Network Configuration](/docs/evm/developers/local-setup)
